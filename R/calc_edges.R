...
#' 
#' @description A vector feature 'edges' is derived from 'streams_v' and several
...
#'  \item{Calculation of segments upstream distance ("upDist") = flow length
...
#'  \item{Calculation of catchment areas, "H2OArea" = total catchment area of each
#'  segment}
#' }
#'
#' @param clean logical; Should intermediate layer (rca) be removed from GRASS session?
...
#' Intermediate layers cleaned is:
#' \itemize{
...
calc_edges <- function(clean = TRUE) {
  ...
  dt.streams[, ':=' (cat = as.numeric(cat), stream = as.numeric(stream), next_stream = as.numeric(next_stream), prev_str01 = as.numeric(prev_str01), prev_str02 = as.numeric(prev_str02)) ]
  a<-data.table(as.numeric(areas[,1]),as.numeric(areas[,2]))
  names(a)<-c("cat","area")
  dt.streams<-merge(dt.streams,a,by="cat",all=T)  # MiKatt: must be 'cat' not 'stream' because stram_r is based on 'cat'!
  setkey(dt.streams,stream)
  # set watershed area of short segments that do not have a rac (NA) to zero (mainly resulting form correct_compl_junctions())
  ...
  dt.streams[,area := area / 1000000]
  dt.streams[,total_area := total_area / 1000000]
  ...
  execGRASS("db.droptable", flags = "quiet",
            parameters = list(
