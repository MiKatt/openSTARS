...
## Error message in import_data()
**_Problem_** Error message in R console when importing data: 

```{r import, error = TRUE, eval = TRUE}
pred_path <- system.file("extdata", "nc", "landuse.shp", package = "openSTARS")
import_data(dem = dem_path, sites = sites_path, predictor_vector = pred_path)
```

Usually, the import works correctly, nevertheless. Please check, if all files are in the mapset:
```{r check import, error = TRUE, eval = TRUE}
execGRASS("g.list", parameters = list(
  type = "raster,vector"
))
```
